...
export class SearchModal extends SuggestModal<Article> {
  ...
  async getSuggestions(query: string): Promise<Article[]> {
    ...
    const searchResponse = (
      await requestUrl(
        baseURL + `&action=opensearch&profile=fuzzy&search=${query}`
      ).catch((e) => null)
    )?.json;
    ...
    const descriptionResponse = (
      await requestUrl(
        baseURL +
          `&action=query&prop=description&titles=${searchResponse[1].join("|")}`
      ).catch((e) => null)
    )?.json;
    const descriptions = Object.values(descriptionResponse.query.pages).map(
      (page: any) => ({
        title: page.title,
    ...
    if (!searchResponse || !descriptions)
      return [
        {
          title:
            "An error occurred. You should check your internet connection!",
          url: "",
          description: "",
        },
      ];
